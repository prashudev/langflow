...
Create a folder that follows the same structural conventions as the [config.yaml](https://github.com/prashudev/langflow/blob/dev/src/backend/langflow/config.yaml) file. Inside this main directory, use a `custom_components` subdirectory for your custom components.
